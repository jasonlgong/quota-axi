...
- The truecolor and 256-color palettes come in a dark (Catppuccin Mocha) and a light (Catppuccin Latte) theme. `--theme light|dark|auto` picks one; when the flag is absent `QUOTA_AXI_THEME` (same values) applies, and the default is `auto`. `auto` reads `COLORFGBG` (`fg;bg`): a background index of 7-15 selects `light`, anything else, absent, or unparseable selects `dark`, so a terminal that sets nothing renders exactly as before. quota-axi never sends an OSC 11 background query, which would add a terminal round-trip to every live refresh. An invalid flag or variable value is a usage error, not a silent fallback. The ANSI-16 path is theme-independent and always uses the terminal's own colors.
